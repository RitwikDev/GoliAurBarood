...
{
    private const float FIRE_ANIMATION_TIME = 0.01f;
    ...
    private int invincibilitySpriteIndex = 0;
    private bool first;
    ...
    private SpriteRenderer invincibilitySpriteRenderer;
    ...
    private void Awake()
    {
        ...
        invincibilitySpriteRenderer = transform.Find(Properties.PLAYER_INVINCIBILITY_BARRIER).GetComponent<SpriteRenderer>();
    ...
    private void Update()
    {
        ...
        if (player.isRespawnInvincible)
            SetRespawnInvincibilitySprite();
        
        if (player.isInvincible)
            SetInvincibilitySprite();

        if(!player.isInvincible && !player.isRespawnInvincible)
            SetOpaqueColour();
    }
    ...
    private void Die()
    {
        ...
        spriteRenderer.sprite = spritesDie[spriteIndex];

        if (spriteIndex == spritesDie.Length - 1)
            return;

        spriteIndex = (spriteIndex + 1) % spritesDie.Length;
    ...
    private void SetRespawnInvincibilitySprite()
    {
        respawnInvincibilityTimer = respawnInvincibilityTimer + Time.fixedDeltaTime;
        if(respawnInvincibilityTimer >= animationTime)
        {
            respawnInvincibilityTimer = 0;
            spriteRenderer.color = (spriteRenderer.color.a != 1)? new Color(1, 1, 1, 1) : new Color(1, 1, 1, 0.2f);
        }
    }

    private void SetInvincibilitySprite()
    {
        invincibilityTimer = invincibilityTimer + Time.fixedDeltaTime;
        if (invincibilityTimer >= FIRE_ANIMATION_TIME)
        {
            invincibilityTimer = 0;
            invincibilitySpriteRenderer.sprite = spritesFire[invincibilitySpriteIndex];
            invincibilitySpriteIndex = (invincibilitySpriteIndex + 1) % spritesFire.Length;
        }
    }

    private void SetOpaqueColour()
    {
        spriteRenderer.color = new Color(1, 1, 1, 1);
        invincibilitySpriteRenderer.sprite = null;
        respawnInvincibilityTimer = 0;
        invincibilityTimer = 0;
    }
}