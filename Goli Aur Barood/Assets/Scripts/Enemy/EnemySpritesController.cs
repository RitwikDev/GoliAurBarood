...
public class EnemySpritesController : MonoBehaviour
{
    ...
    public Sprite[] spritesRunDiagonalUp1 = new Sprite[1];
    public Sprite[] spritesRunDiagonalUp2 = new Sprite[1];

    public Sprite[] spritesRunDiagonalDown1 = new Sprite[1];
    public Sprite[] spritesRunDiagonalDown2 = new Sprite[1];
    ...
    private EnemyGroundCheck enemyEnemyGroundCheck;
    ...
    private void Awake()
    {
        ...
        enemyEnemyGroundCheck = transform.Find(Properties.GROUND_CHECK_NAME).gameObject.GetComponent<EnemyGroundCheck>();
    ...
    private void FixedUpdate()
    {
        ...
        {
            if (enemy.shootStraight || enemy.isFalling)
                StandShoot();

            else if (enemy.shootDiagonalUp)
                RunDiagonalUp();

            else if (enemy.shootUp)
                Up();

            else if (enemy.shootDiagonalDown)
                RunDiagonalDown();

            else if (enemy.runStraight && !enemy.isFalling)
                RunIdle();
    ...
    private bool checkAnimationChange(SpriteName spriteName)
    {
        ...
        {
            if (enemyFire.recoil && first)
            {
                first = false;
                SetRecoilSprite();
            }

            else if(!enemyFire.recoil && !first)
            {
                first = true;
                SetRecoilSprite();
            }
